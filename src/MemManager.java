...
public class MemManager {
	...
	private byte[] memoryPool; // byte array the holds seminar information in byte form
	private FreeList freeList; // freelist that determines free space in the byte array 
	
	/**
     * Constructs a new MemManager with a specified memory pool size.
     * It also initializes the free list with the highest power of 2 equal to poolsize.
     *
	...
	public MemManager(int poolsize) { 
		...
		int sizePower = (int) (Math.log(poolsize) / Math.log(2)); // finds highest power of 2 equal to poolsize 
	...
	/**
     * Doubles the size of the memory pool, copying existing data to the new, larger pool.
     */
	...
	/**
     * Inserts a new record into the memory pool, returning a handle to its location.
     *
	...
	/**
     * Removes the record associated with the given handle, resetting its bytes to zero 
     *
	...
	public void remove(Handle theHandle) { 
	 ...
	 /**
     * Retrieves a record associated with the given handle, copying up to a specified 
     * number of bytes into a provided array.
     *
